import { useState } from 'react';
  ...
  const AnectdoteMostVotes = ({vote, anecdotes}) => {
    let number = 0;
    const sum = Object.values(vote).reduce((a, b)=> a+b, 0);
    for (const i in anecdotes) {
//      console.log(i)
      if (number < vote[i]) {
        number = i;
...
const App = () => {
  ...
  return (
    <div>
      ...
      <AnectdoteMostVotes vote={vote} anecdotes={anecdotes}/>
    </div>
  );
}
